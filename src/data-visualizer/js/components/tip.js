...
import { DatumKeys } from './../util/helpers.js';
...
class Tip extends Component {
	...
	getCall() {
		...
		return d3
			...
			.direction(
				dv.args('tooltipDirectionParameter') || dv.args('tooltipDirection')
			)
			.html(Tip.html);
	}
				...
				if (typeof dis[prop] !== 'object') {
					...
					DatumKeys.forEach((keyKey) => {
						if (
							...
							html += `<span class="${dv.createClass(
								'tooltip-data-property-content'
							)}">${dv.format(keyKey)(
								dv._get(dis, dv.args(`key['${keyKey}' ]`))
							)} </span>`;
