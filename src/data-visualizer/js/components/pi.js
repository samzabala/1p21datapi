...
const OPTIMAL_PI_MULTIPLIER = 0.5;
const OPTIMAL_PI_LEGENDARY_MULTIPLIER = 0.375;
...
class Pi extends Component {
	...
	get radius() {
		...
		let value = Math.min(dv.args('width') * 0.5, dv.args('height') * 0.5);
	...
	path(calcWithInnerRadius, subMethod, offsetMultiplier, initial) {
		...
		const innerRadius = calcWithInnerRadius
			? this.radius * dv.args(`piInRadius`)
			: 0;
		const outerRadius =
			!initial ||
			(initial && offsetMultiplier <= 1 && calcWithInnerRadius == false)
