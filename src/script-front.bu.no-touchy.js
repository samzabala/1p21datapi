...
(function (window, d3) {
	...
	_1p21.dataVisualizer = function (selector, arr) {
		...
		var coordinates = ['x', 'y'],
			...
			getAxisStringOppo = function (axisString) {
		...
		_.has_text = (function () {
			if (
				...
				((args.type !== 'pie' &&
					...
					(args.type == 'pie' && (args.piLabelStyle || !args.colorLegend)))
			) {
				return true;
			} else {
				return false;
		...
		_.has_both_text_on_shape = (function () {
			if (
				_.has_text &&
				((args.type !== 'pie' && !args.xTicks && !args.yTicks) ||
					(args.type == 'pie' &&
						!args.colorLegend &&
		...
		var deepGet = function (obj, keyString, isNum) {
			...
			if (isNum == true && isNaN(value)) {
				console.warn(
					selector +
						' data with the key source of `' +
						keyString +
						'` was passed as numeric but is not.'
				);
			}
			return value;
		};
		...
		var getRange = function (key) {
			...
			switch (key) {
				...
				case 1:
					if (
						args[getAxisStringOppo(getAxisString(key)) + 'Align'] ==
							'top' ||
			...
			if (coordinateAttribute == 'x') {
				...
				if (axisString == 'x') {
					if (args[axisString + 'Align'] == 'bottom') {
						offset =
							args[getDimension(axisString, true)] +
							_.margin.bottom * 0.875;
					} else {
						offset = -(_.margin.top * 0.875);
					}
				} else if (axisString == 'y') {
					if (args[axisString + 'Align'] == 'right') {
						offset =
							args[getDimension(axisString, true)] +
							(_.margin.right * 0.875 + _.text_base_size);
					} else {
						offset = -(_.margin.left * 0.875 - _.text_base_size);
					}
				}
			}
		...
		var getShapeTextBaselineShift = function (coordinateAttr, keyKey) {
			...
			if (_.has_both_text_on_shape) {
				if (coordinateAttr == 'y' && _.has_both_text_on_shape) {
					// calculate height
					var full_height =
						_.text_base_size *
						(args.textNameSize + args.textValueSize + _.text_padding); // .5 margin top bottom and between text

					if (keyKey == 1) {
						shift =
							(full_height * -0.5 +
								_.text_base_size * args.textValueSize * 0.5 +
								_.text_base_size) /
						...
						shift =
							(full_height * 0.5 -
								_.text_base_size * args.textNameSize * 0.5 -
								_.text_base_size) /
			...
			if (args.type == 'pie') {
				...
				var shiftPad = function () {
						...
						if (!(initial || args.type == 'scatter')) {
							if (
								args[getAxisStringOppo(coordinate) + 'Align'] ==
									'bottom' ||
								args[getAxisStringOppo(coordinate) + 'Align'] == 'right'
							) {
								multiplier = -1;
							...
							if (keyKey !== 0 && coordinate == 'x') {
								value = _.text_padding * 0.5 * _.text_base_size;
							}
								...
								if (coordinate == 'x') {
									if (
										...
										parseFloat(getShapeSize(coordinate, dis, i)) >=
											args[getDimension(coordinate, true)] -
									...
									if (
										...
										value = _.m_length(coordinate, i) * -0.5;
									} else {
										value = _.m_length(coordinate, i) * 0.5;
									}
								}
				...
				if (args.nameIsNum == true || keyKey == 1) {
					if (
						...
						if (args.type == 'line' || args.type == 'scatter') {
							if (!(initial && keyKey !== 0 && args.type !== 'scatter')) {
								offset = _['the_' + args[coordinate + 'Data']](
									deepGet(dis, args.key[keyKey], true)
								);
							}
						}
					...
					offset = _['the_' + args[coordinate + 'Data']](
						deepGet(dis, args.key[keyKey], false)
					);
		...
		var getLegendOrigin = function (axisString) {
			if (_.container_legend) {
				var offset = 0,
					length =
						axisString == 'x'
							? _.container_legend.nodes()[0].getBoundingClientRect()[
									getDimension(axisString)
							  ]
							: _.legend_height, // .8
					shifter = function () {
						var value = 0,
						...
						if (
							args.type == 'pie' ||
							(args.type !== 'pie' &&
								args[getAxisStringOppo(axisString) + 'Align'] ==
									'left') ||
				...
				if (
					...
					offset = args[getDimension(axisString)] * 0.5;
				}
			...
			if (pathInitiator == 'area') {
				...
				var multiplier = (function () {
					...
					if (args.lineFillInvert) {
						toReturn =
							args[axisToFill + 'Align'] == 'top' ||
							args[axisToFill + 'Align'] == 'left'
								? 1
								: -1;
					}
				...
				path[aCord.name](function (dis, i) {
					...
					[aCord.value](function (dis, i) {
						return getShapeOrigin(
							getAxisStringOppo(axisToFill),
							dis,
							i,
							initial
						);
					...
					[aCord.fill](function (dis, i) {
						return (
							...
							multiplier * args[getDimension(axisToFill)]
						);
					});
			} else {
		...
		var getArcPath = function (
			...
			var innerRadius = (function () {
				...
				path = d3.arc().outerRadius(outerRadius).innerRadius(innerRadius);
		...
		var getPiOrigin = function (axisString) {
			...
			if (args.colorLegend && axisString == 'x') {
				offset = args[getDimension(axisString)] * 0.375;
			} else {
				offset = args[getDimension(axisString)] * 0.5;
			}
		...
		var setScale = function (keyKey) {
			...
			switch (keyKey) {
				case 'color':
					scale = d3.scaleOrdinal().range(_['range_' + keyKey]);
					break;
				...
				case 1:
					if (args.nameIsNum == true || keyKey == 1 || keyKey == 'area') {
						if (
							...
							scale = d3.scaleLinear().range(_['range_' + keyKey]);
						}
					} else {
						if (args.type == 'line' || args.type == 'scatter') {
							scale = d3
		...
		var renderAxisContainers = function (axisString, containerObj, isGrid) {
			if (args[axisString + 'Ticks']) {
				...
				if (args[axisString + 'Label'] && !isGrid) {
					_['lab_' + axisString] = _.container_lab
						.append('text')
					...
					_['lab_' + axisString].transition(_.duration).attr('opacity', 1);
				}

				//ruler/grid
				_[tickContainer] = containerObj.append('g').attr('class', function () {
					var contClass = null;
		...
		var setAxis = function (axisString, isGrid) {
			...
			var axisToReturn = d3[axisKey.toCamelCase()](
				_['the_' + args[axisString + 'Data']]
			);

			if (args[axisString + 'Ticks']) {
				if (
					args.type == 'scatter' &&
					args[axisString + 'Data'] == 0 &&
					args.nameIsNum == true
				) {
				...
				if (isGrid) {
					...
					axisToReturn.tickFormat(function (dis, i) {
		...
		var renderCursorStalker = function (d3_event) {
			return _.tooltip_cursor_stalker
				.attr(
					'cx',
					d3_event.offsetX * (_.outer_width / _.svg.node().clientWidth) + 'px'
				)
				.attr(
					'cy',
					d3_event.offsetY * (_.outer_height / _.svg.node().clientHeight) +
						'px'
				)
				.node();
		};
		...
		var renderGraph = function (incomingData) {
				...
				coordinates.forEach(function (coordinate) {
					if (args[coordinate + 'Ticks']) {
						...
						if (args[coordinate + 'Grid']) {
							_['grid_' + coordinate + '_axis'] = setAxis(
								coordinate,
								true
							);
			...
			_.shape = _.container_graph
				.selectAll(
					_.graph_item_element + '.' + prefix + 'graph-item.graph-item-shape'
				)
			...
			//text exit
			if (_.has_text) {
				_.shape_text = _.container_graph
					.selectAll('text.' + prefix + 'graph-item.graph-item-text')
					.data(_.data, function (dis) {
						return deepGet(dis, args.key[0]);
					});
				...
				if (args.type == 'pie' && args.piLabelStyle == 'linked') {
					_.shape_text_link = _.container_graph
						.selectAll('polyline.' + prefix + 'graph-item.graph-item-link')
						.data(_.data, function (dis) {
							return deepGet(dis, args.key[0]);
						});
				...
				if (args.type == 'line') {
					...
					_.line = _.container_graph
						...
						.attr(
							...
							prefix +
								'line' +
								(args.lineColor !== null ? ' has-color' : ' no-color')
						)
						.attr('fill', 'none')
					...
					if (args.lineColor) {
						_.line.attr('stroke', args.lineColor);
					}

					if (args.lineFill) {
						_.fill = _.container_graph
							.select('.' + prefix + 'fill')
							.remove();
						...
						_.fill.transition(_.duration).attrTween('d', function () {
							return getInterpolation(
						...
						if (args.lineFillColor || args.lineColor) {
							_.fill.attr('fill', args.lineFillColor || args.lineColor);
						}
					}
				}
			}
			...
			_.enter_shape = _.shape
				.enter()
				.append(_.graph_item_element)
				.attr('class', function (dis) {
					return (
						prefix +
						'graph-item graph-item-shape' +
						' ' +
						'data-name-' +
						deepGet(dis, args.key[0])
					);
				});
			...
			//areas and what not
			if (args.type == 'pie') {
				_.merge_shape.transition(_.duration).attrTween('d', function (dis, i) {
			...
			if (args.type == 'line' || args.type == 'scatter') {
				_.merge_shape.transition(_.duration).attrTween('r', function (dis, i) {
					return getInterpolation(
				...
				if (args.type == 'line' && !args.linePoints) {
					_.merge_shape.attr('fill-opacity', 0).attr('stroke-opacity', 0);
				}
			} else {
				_.merge_shape
					.transition(_.duration)
			...
			if (!args.colorPalette.length) {
				if (args.type == 'line' && (args.linePointsColor || args.lineColor)) {
					_.merge_shape.attr('fill', function () {
						return args.linePointsColor || args.lineColor;
					});
				}
			} else {
				_.merge_shape.attr('fill', function (dis, i) {
			...
			if (_.has_text) {
				...
				if (_.shape_text_link) {
					...
					_.merge_shape_text_link = _.shape_text_link
						.merge(_.enter_shape_text_link)
						.transition(_.duration)
						.attrTween('stroke-opacity', function (dis, i) {
							return getInterpolation(0, 0.75);
						})
				...
				_.enter_shape_text = _.shape_text
					...
					.attr('stroke', function (dis) {
						if (
							...
							(args.type == 'pie' && args.piLabelStyle == 'linked')
						) {
				...
				[0, 1].forEach(function (keyKey) {
					if (
						...
						_['shape_text_' + keyKey] = _.enter_shape_text
							.append('tspan')

							.attr('class', function (dis) {
								return (
									'graph-item-text-data-' +
									keyKey +
									' ' +
									'data-name-' +
									deepGet(dis, args.key[0])
							...
							.attr('font-size', function () {
								...
								if (
									(args.type !== 'pie' &&
										!args[
											getAxisStringOppo(getAxisString(keyKey)) +
												'Ticks'
										]) ||
									(args.type == 'pie' &&
										!args.colorLegend &&
				...
				_.m_length = function (axisString, i) {
					if (_.has_text && _.merge_shape_text) {
						...
						if (getDimension(axisString) == 'width') {
							value =
								_.merge_shape_text.nodes()[i].getBBox()[
									getDimension(axisString)
								] +
								_.text_padding * _.text_base_size;
						} else {
							value =
								_.text_base_size *
								(args.textNameSize +
						...
						if (
							(args.type == 'bar' &&
								((args.barTextWithin &&
									(// it out
									(parseFloat(
										getShapeSize(getAxisString(1), dis, i, false)
									) < _.m_length(getAxisString(1), i) &&
										!isDark(args.colorBackground)) || //it in
										(parseFloat(
											getShapeSize(
												getAxisString(1),
												dis,
							...
							(args.type == 'pie' &&
								((args.piLabelStyle == 'within' &&
									args.colorPalette.length > 0 &&
									!isDark(
										_.the_color(deepGet(dis, args.key.color))
									)) ||
			...
			if (args.colorLegend) {
				...
				_.dom_color.forEach(function (key, i) {
					...
					_.legend
						...
						.attr('width', _.legend_size * 0.75)
						.attr('height', _.legend_size * 0.75)
					...
					_.legend
						...
						.attr('y', _.legend_size * 0.375)
						.attr('stroke', args.colorBackground);

					_.legend.attr(
						'transform',
						'translate(0, ' + i * _.legend_size + ')'
					);
		...
		var init = function (retrievedData) {
			...
			_.the_container
				.classed(
					prefix +
						'initialized' +
						(isDark(args.colorBackground) ? ' ' + prefix + 'dark' : ''),
					true
				)
						...
						if (_.user_can_debug) {
							...
							console.warn(
								selector +
									' datum index `' +
									i +
									'` was filtered.\ndatum does not have data for the key `' +
									args.key[keyKey] +
									'`, which is set as the property for `' +
									humanForKey +
									'`'
							);
						}
			...
			if (_.data.length > 0) {
				...
				if (args.nameIsNum == true) {
					...
					sortable.sort(function (a, b) {
						return (
							deepGet(a, args.key[0], true) -
							deepGet(b, args.key[0], true)
						);
					});
					...
					function (dis, i) {
						...
						for (var prop in dis) {
							if (Object.prototype.hasOwnProperty.call(dis, prop)) {
								//item
								html +=
									'<div class="' + prefix + 'tooltip-data-property">';
				...
				if (args.description) {
					_.heading_description = _.heading_sel
						.append('span')
				...
				document.addEventListener(
					...
					function (e) {
						...
						if (graphPosition < window.innerHeight * 0.5 && !_.dv_init) {
							_.dv_init = true;
							...
							setTimeout(function () {
								...
								if (args.tooltipEnable) {
									...
									_.tooltip_cursor_stalker = _.svg.select(
										'circle.' + prefix + 'cursor-stalker'
									);
									...
									_.tooltip = d3
										...
										.style('width', function () {
											if (typeof args.tooltipWidth === 'number') {
												return (
													parseFloat(args.tooltipWidth) + 'px'
												);
										...
										.direction(
											args.tooltipDirectionParameter ||
												args.tooltipDirection
										)
										.html(_.tooltip_html);
								...
								if (args.type == 'pie') {
									...
									_.container_lab = _.container
										.append('g')
										.attr('class', prefix + 'label');

									// container for axis
									_.container_rule = _.container
										.append('g')
									...
									if (args.xGrid || args.yGrid) {
										_.container_grid = _.container
											.append('g')
											.attr('class', prefix + 'grid')
											.attr(
												'font-size',
												args.textTicksSize + 'em'
											);
									}
								}

								//create container for graph
								_.container_graph = _.container
									.insert('g')
								...
								if (_.user_can_debug) {
									...
									if (
										JSON.stringify(args.margin) ==
											JSON.stringify(defaults.margin) &&
										(args.xLabel || args.yLabel) &&
										(args.xTicks || args.yTicks)
									) {
										console.debug(
											selector +
												' text may overlap. margins may need to be modified'
										);
									}
								}
								...
								datum_keys.forEach(function (keyKey) {
									...
									switch (keyKey) {
										case 0:
										case 1:
											renderAxisContainers(
												getAxisString(keyKey),
												_.container_rule
											);
				...
				renderError(
					'Data was filtered and all items are invalid for visualizing. check provided data keys and make sure they are correct'
				);
			}
		};
		...
		if (args.srcPath.indexOf(window.location.href) > -1) {
			var jsonSelector = document.getElementById(
				args.srcPath.getHash()
			).innerHTML;
